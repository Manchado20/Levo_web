...
export class AuthUtils
{
     ...
     * @returns
     */
    static getAccessTokenPayload(token: string) : any
    {
        return this._decodeToken(token);
    }

    /**
     * Verify if the current access token stored in localstorage is valid.
     *
     * @returns
     */
    static isValidAccessToken(token: string)
    {
        if(!token || this.isTokenExpired(token) || !this.getAccessTokenPayload) {
